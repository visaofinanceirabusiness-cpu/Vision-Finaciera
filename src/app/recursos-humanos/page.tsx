...
export default function RecursosHumanosPage() {
  ...
  async function cargarDatos() {
    ...
    const { data: userData, error: errorUsuario } =
    ...
    const [
      ...
      supabase
        .from('clientes')
        .select(
          'id, empresa_id, nombre, telefono, direccion, creado_en, codigo, fecha_alta'
        )
    ...
    setClientes(
      ordenarPersonas(
        (clientesData ?? []) as Persona[]
      )
    );

    setProveedores(
      ordenarPersonas(
        (proveedoresData ?? []) as Persona[]
      )
    );
      ...
      >
        ...
        <main style={panel}>
          ...
          <input
            ...
            placeholder={
              pestana === 'clientes'
                ? 'Buscar código, nombre, teléfono o dirección...'
                : 'Buscar código, nombre, teléfono o dirección...'
            }
            style={inputBusqueda}
          />
...
function generarProximoCodigo(
  ...
  const numeroFormateado =
    String(siguiente).padStart(
      5,
      '0'
    );

  return tipo === 'clientes'
    ? `CLI-${numeroFormateado}`
    : `PROVE-${numeroFormateado}`;
}

function extraerNumeroCodigo(
  codigo: string | null
): number {
  if (!codigo) {
    return 0;
...
const botonNuevo: React.CSSProperties = {
  ...
  fontWeight: 800,
  boxShadow:
    '0 8px 20px rgba(46,139,87,0.20)',
};
